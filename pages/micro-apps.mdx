...
This tutorial will go over how to build micro applications for your Shopify shop. We will be building the tracking application that is currently on the app.
...
<!-- prettier-ignore -->
The submission form has an input field and a submit button. We are using 3 states, input value,result, and loading. On submit, we are calling the getTracking funciton.
...
- The url is passed as a prop to the component and we simply pass it the function. The url is the endpoint where your shop's graphQL API is mounted.
...
We simply post a graphQL query to the endpoint and set the results to our state. You can use the Playground to create and test different queries and mutations. You can click DOCS button on the Playground to get the full Shopify graphQL API spec.
